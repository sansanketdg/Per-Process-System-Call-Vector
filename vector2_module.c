...
char vector_name[MAX_VECTOR_NAME_LEN] = "Vector968";
...
static int initVector(void)
{
	...
	ret = add_syscall_to_vector(__NR_mkdir, (unsigned long)overriddenMkdir);
	if (ret < 0) {
		goto out;
	}
	ret = add_syscall_to_vector(__NR_rmdir, (unsigned long)overriddenRmdir);
	if (ret < 0) {
		goto out;
	}
	...
	ret = add_syscall_to_vector(__NR_open, (unsigned long)blockedOpen);
	if (ret < 0) {
		goto out;
	}
	ret = add_syscall_to_vector(__NR_close, (unsigned long)overriddenClose);
	if (ret < 0) {
		goto out;
	}
	bitmap = MKDIR_OVERRIDE + RMDIR_OVERRIDE + WRITE_LOGGED + OPEN_BLOCKED + CLOSE_OVERRIDE;
